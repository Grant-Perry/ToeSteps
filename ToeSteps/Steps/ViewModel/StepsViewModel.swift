...
class StepsViewModel: ObservableObject {
   ...
   func formattedDistance(_ meters: Double) -> String {
	  ...
	  return distanceFormatter.string(from: NSNumber(value: miles)) ?? "0.0"
   }
   ...
   func distanceForDate(_ date: Date) -> Double {
	  let dayStart = Calendar.current.startOfDay(for: date)
	  return distanceData[dayStart] ?? 0.0
   }
   ...
   func fetchStepsData(from startDate: Date? = nil, to endDate: Date? = nil) {
	  ...
	  DispatchQueue.global(qos: .userInitiated).async {
		 ...
		 let group = DispatchGroup()
			...
			// Fetch steps data
		 group.enter()
		 self.fetchStepsDataOnly(from: midnightOfStartDate, to: endOfDay) {
			group.leave()
		 }

			// Fetch distance data
		 group.enter()
		 self.fetchDistanceData(from: midnightOfStartDate, to: endOfDay) {
			group.leave()
		 }
   ...
   private func fetchStepsDataOnly(from startDate: Date, to endDate: Date, completion: @escaping () -> Void) {
   ...
   private func fetchDistanceData(from startDate: Date, to endDate: Date, completion: @escaping () -> Void) {
	  ...
	  query.initialResultsHandler = { _, result, error in
		 ...
		 var newDistanceData: [Date: Double] = [:]
		 result.enumerateStatistics(from: startDate, to: endDate) { statistics, _ in
			if let sum = statistics.sumQuantity() {
			   let distance = sum.doubleValue(for: HKUnit.meter())
			   let dayStart = Calendar.current.startOfDay(for: statistics.startDate)
			   newDistanceData[dayStart] = distance
		 ...
		 DispatchQueue.main.async {
			if self.isExtendedDataMode {
				  // Merge new data with existing data
			   for (date, distance) in newDistanceData {
				  self.distanceData[date] = distance
			   }
			} else {
				  // Normal mode, replace all data
   ...
   private func refreshTodaySteps() {
	  ...
	  let distanceQuery = HKStatisticsQuery(quantityType: distanceQuantityType,
		 ...
		 if let result = result, let sum = result.sumQuantity() {
			...
			DispatchQueue.main.async {
			   self.todayDistance = distance
			   self.distanceData[startOfToday] = distance
			}
		 }
	  }
